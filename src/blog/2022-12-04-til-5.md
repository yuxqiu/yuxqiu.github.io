...
5. Somethings about the test
   - You need to set environment variable `JAZZER_FUZZ` to any non-empty value; otherwise, only empty input will be given to your function.
   - "When an assertion in the test fails, an exception is thrown but not caught, or Jazzer's instrumentation detects a security issue (e.g. SQL injection or insecure deserialization), the fuzz test is reported as failed and the input is collected in the inputs directory for the test class."
   - Only a single fuzz test per test run will be executed in fuzzing mode. All other fuzz tests will be skipped.
      - This means if you want to run all the fuzzing tests inside some packages, you need to run them one by one.
...
- Prefer default argument rather than overloading

- Initialization order is the order the var is defined
   - So, to avoid confusion, we need to use the same order in constructor
   - Even if we declare them in different order, they are actually evaluated in the order in which the member variables are defined

- `std::tie` and `std::ignore`
   - `std::tie` creates a tuple of references
   - we can use this to bind a returned tuple to some of the existing variable
   - `std::ignore` can be used with `std::tie` to let us ignore some variables


## Trivia

- `std::nextafter`
   - Useful if we want to get the next value of `float/double/long double`
   - [C++ Weekly - Ep 223 - Know Your Standard Library: std::nextafter](https://www.youtube.com/watch?v=-F0j2VN4xEU)

- `std::recursive_mutex`
   - Useful when we need to 1) acquire a lock and 2) do some recursions in that function
   - Solutions without `recursive_mutex` is to create a entry function and do locking in it. Check [this answer](https://stackoverflow.com/a/2415088).

- Inefficient placement new before C++17[^1]
   - For any version of gcc without -std=c++17 or -std=c++1z, any version of clang below 3.4 (January 2014)
   - Placement new will perform a null pointer check on the memory passed in. If null is passed in: the returned object is also null
   - Marc Glisse/Jonathan Wakely (wg21.link/cwg1748) clarified this in 2013: Passing null to placement new is now Undefined Behaviour [5.3.4.15]

- Profilers[^1]
   - I have talked about profilers [before](/blog/2022/pprof/). This is an example of sampling profiler. The main drawbacks they have is that "they miss the key events".
   - There are some other profilers like: Instrumentation profilers (e.g. callgrind)
