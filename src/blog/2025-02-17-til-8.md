...
date = 2025-02-17
taxonomies = { tags = ["til", "cryptography", "algorithm", "systems"] }
+++
...
![Figure showing four random walks (trails) converging to a collision point, illustrating parallelized Pollard's rho collision search, with distinguished points marked as open circles](/img/blog/2025/02/parallel-pollard-rho.png)
...
![Diagram of a logically ordered cache queue showing insertion, eager and lazy promotion, passive and quick demotion, and removal of entries](/img/blog/2025/02/cache-abstraction.png)
*Cache Abstraction*
...
![Illustration of Earth with satellites positioned along the equatorial plane, showing that signals from these satellites cannot distinguish between a point and its mirror image across the equator](/img/blog/2025/02/problem-of-geostationary-satellites.png)
*If all GPS satellites are geostationary, we cannot differentiate the north and south atmosphere.*

![Diagram of multiple satellite orbital planes around Earth, with a heat-map overlay showing the number of visible satellites, ranging from 6 to 16, at each point on the globe](/img/blog/2025/02/number-of-satellites.png)
*The number of visible satellites from every point on Earth, with density.*

![Step-by-step diagram of GPS signal decoding: the carrier-removed signal, a code replica, their product, the summed areas, and the difference of summed areas used to recover the satellite's source data](/img/blog/2025/02/decode-signal.png)
